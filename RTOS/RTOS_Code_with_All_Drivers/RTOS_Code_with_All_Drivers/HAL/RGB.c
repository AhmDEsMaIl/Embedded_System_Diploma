...
void  Set_RGB(Colors C)
{
	...
	{
		DIO_WritePin(PIND5_PR,HIGH);
	} 
	else if((Color[C].Green)!=255)
	{
		OCR1A_PR=Color[C].Green;
	}
	if(Color[C].Bule==255)
	{
		DIO_WritePin(PIND4_PR,HIGH);
	}
	else if((Color[C].Bule)!=255)
	{
		OCR1B_PR=Color[C].Bule;
	}
...
void Show_off_interput()
{
	...
	if(f2==0)
	{
	if(OCR1A_PR>Color[I2].Green)
	{
		OCR1A_PR--;
		/*LCD_SetCursor(1,0);
		LCD_WriteNumber(OCR1A);*/
	}
	else if(OCR1A_PR<Color[I2].Green)
	{
		OCR1A_PR++;
		/*LCD_SetCursor(1,6);
		LCD_WriteNumber(OCR1A);*/
	...
	if(f3==0)
    {
	if(OCR1B_PR>Color[I2].Bule)
	{
		OCR1B_PR--;
	}
	else if(OCR1B_PR<Color[I2].Bule)
	{
		OCR1B_PR++;
	}
	else if(OCR1B_PR==Color[I2].Bule)
	{
		OCR1B_PR=Color[I2].Bule;
		f3=1;
